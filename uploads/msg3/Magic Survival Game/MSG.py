...
def check_assets():
 ...
 pygame.init()
 def c(n,c,s=(40,40),r=0):surf=pygame.Surface(s,pygame.SRCALPHA);(pygame.draw.circle(surf,c,(s[0]//2,s[1]//2),s[0]//2)if r else surf.fill(c));pygame.image.save(surf,os.path.join(ASSETS_DIR,f"{n}.png"))
 ...
 pygame.image.save(wizard_surf, os.path.join(ASSETS_DIR, "wizard.png"))
...
class GameManager:
 def __init__(self):
  ...
  for a in["bullet","orb","legendary"]:
   try:self.assets[a]=pygame.image.load(os.path.join(ASSETS_DIR,f"{a}.png")).convert_alpha()
  ...
  sound_files={"firebreath":os.path.join(SOUNDS_DIR,"firebreath_burst.wav"),"level_up":os.path.join(SOUNDS_DIR,"level_up_chime.wav"),"game_over":os.path.join(SOUNDS_DIR,"game_over.wav"),"victory":os.path.join(SOUNDS_DIR,"victory.wav")}
 ...
 def spawn_final_boss(self):
  ...
  try:
   boss_music_path=os.path.join(SOUNDS_DIR,"boss_music.wav")
   if os.path.exists(boss_music_path):self.boss_music=pygame.mixer.Sound(boss_music_path);self.boss_music.play(-1)
   else:print(f"Boss music file not found: {boss_music_path}")
  except Exception as ex:print(f"Error loading boss music: {ex}")
  self.enemies.add(e)
 ...
 def playing_loop(self):
  ...
  if self.game_time>=420000 and not hasattr(self,"final_boss_spawned"):print(f"Boss spawn triggered at {self.game_time}ms");self.final_boss_spawned=True;self.spawn_final_boss()
  ...
  while 1:
   ...
   for event in pygame.event.get():
    ...
    if event.type==pygame.KEYDOWN:
     ...
     if event.key==pygame.K_m and self.creative_mode:self.player.skills={"Firebreath":7,"Lightning":7};self.game_time=420000;self.final_boss_spawned=True;self.spawn_final_boss()
